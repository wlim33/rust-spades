...
async fn anon_claim_attaches_seats() {
    ...
    let bob_id_s = reg.json::<serde_json::Value>()["id"]
        .as_str()
        .unwrap()
        .to_string();
...
async fn claim_with_wrong_anon_id_is_noop() {
    ...
    reg.assert_status(axum::http::StatusCode::CREATED);
    let alice_id =
        uuid::Uuid::parse_str(reg.json::<serde_json::Value>()["id"].as_str().unwrap()).unwrap();
    ...
    let claimed = env
        .store
        .claim_anon_game_seats(uuid::Uuid::new_v4(), alice_id)
        .unwrap();
    assert_eq!(claimed, 0);
