...
use crate::cards::{from_tn, get_trick_winner, new_deck, to_tn, Card, Suit};
...
impl Spades {
    // `new` and `scoring` are called from Task 10's facade; allow until wired up.
    #[allow(dead_code)]
...
mod tests {
    ...
    fn leading_spade_blocked_until_broken() {
        ...
        let spade = tn(Card { suit: Suit::Spade, rank: Rank::Ace });
        let heart = tn(Card { suit: Suit::Heart, rank: Rank::King });
