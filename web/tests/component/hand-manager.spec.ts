...
describe('HandManager', () => {
  ...
  it('mounts the player hand sorted by suit/rank', () => {
    ...
    expect((south.children[0]!.querySelector('img.card-face') as HTMLImageElement).getAttribute('src')).toBe('/cards/AS.svg');
    expect((south.children[1]!.querySelector('img.card-face') as HTMLImageElement).getAttribute('src')).toBe('/cards/2H.svg');
  });
  ...
  it('removeCard removes a specific player card and returns its element', () => {
    ...
    expect((removed?.querySelector('img.card-face') as HTMLImageElement).getAttribute('src')).toBe('/cards/AS.svg');
    expect(south.children.length).toBe(1);
  });
