...
export function scores(p: ScoresProps): TemplateResult {
  const team = (label: string, teamNo: 1 | 2, score: number, bags: number): TemplateResult =>
    html`<span class="spades-scoreboard__team" data-team=${teamNo}>
      <span class="spades-scoreboard__label">${label}</span>
